...
//    _largeLineColor = [UIColor whiteColor];
    _largeLineColor = [UIColor blackColor];
//    _smallLineColor = [UIColor colorWithHexString:@"#FFFFFF" alpha:0.4];
    _smallLineColor = [UIColor colorWithHexString:@"#666666" alpha:1];
    _smallLineHeight = 7;
    ...
    /**   刻度显示文本  */
    _scaleTextColor= [UIColor colorWithHexString:@"#666666"alpha:1];
...
-(void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate{
    if (!decelerate) {
        ...
        count = offset/self.collectionView.frame.size.width;
       // NSLog(@"count ==== %f",count);
        if (self.dividingRulerCustomScaleDidEndScrollingBlock) {
            self.dividingRulerCustomScaleDidEndScrollingBlock(count *self.scalesCountBetweenScaleText);
        }
    }
}
...
-(void)updateRulerLocation{
    ...
    if (self.isShouldAdsorption) {
        count = (NSInteger)(offset/(self.lineWidth+self.lineSpace)+0.5);
        CGFloat temp = count *1.0/self.scalesCountBetweenScaleText;
        NSInteger temp2 = count/self.scalesCountBetweenScaleText;
//        count = (NSInteger)(offset/(self.lineWidth+self.lineSpace)+0.5);
        
        if (temp - temp2 >= 0.5) {
            count = (temp2 +1) * self.scalesCountBetweenScaleText;
        }else if (temp-temp2 < 0.5 && temp-temp2 > 0){
            count = temp2  * self.scalesCountBetweenScaleText;
        }
        
        if (self.dividingRulerValuegBlock) {
            self.dividingRulerValuegBlock(count);
        }
//        NSLog(@">>>>>>>>>>%f",count);
    }else{
        count = offset/(self.lineWidth+self.lineSpace);
...
//        [_currentValueLabel setTextColor:[UIColor whiteColor]];
        [_currentValueLabel setTextColor:[UIColor colorWithHexString:@"#8DAF00" alpha:1]];
...
-(CAGradientLayer *)leftGradientLayer{
    if (!_leftGradientLayer) {
        ...
        _leftGradientLayer.colors = @[(__bridge id)[UIColor colorWithHexString:@"#666666" alpha:0.6].CGColor,(__bridge id)[UIColor colorWithHexString:@"#666666" alpha:0].CGColor];
...
-(CAGradientLayer *)rightGradientLayer{
    if (!_rightGradientLayer) {
        ...
        _rightGradientLayer.colors = @[(__bridge id)[UIColor colorWithHexString:@"#666666" alpha:0.6].CGColor,(__bridge id)[UIColor colorWithHexString:@"#666666" alpha:0].CGColor];
